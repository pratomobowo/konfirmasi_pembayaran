...
@section('content')
    <x-page-header>
        <x-slot name="title">Laporan Tahunan</x-slot>
        <x-slot name="description">
            Laporan pembayaran untuk periode tahunan. Filter berdasarkan tahun untuk melihat ringkasan dan tren pembayaran.
        </x-slot>
    </x-page-header>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="border-t border-gray-200 p-6">
                <form action="{{ route('admin.reports.yearly') }}" method="GET" class="flex items-end space-x-4">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 bg-blue-50">
                ...
                <p class="mt-1 max-w-2xl text-sm text-gray-500">
                    Ringkasan pembayaran bulanan dalam tahun {{ isset($selectedYear) ? $selectedYear : date('Y') }}.
                </p>
            </div>
            <div class="border-t border-gray-200 p-6">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 bg-blue-50">
                ...
                <p class="mt-1 max-w-2xl text-sm text-gray-500">
                    Persentase pembayaran berdasarkan status dalam tahun {{ isset($selectedYear) ? $selectedYear : date('Y') }}.
                </p>
            </div>
            <div class="border-t border-gray-200 p-6">
                    ...
                    <div class="flex items-center justify-center">
                        <div class="space-y-4">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm text-gray-700">Terverifikasi: {{ isset($statusData) ? number_format(($statusData['verified'] / max($totalPayments, 1)) * 100, 1) : 0 }}%</span>
                            </div>
                            <div class="flex items-center">
                                <div class="w-4 h-4 bg-yellow-500 rounded-full mr-2"></div>
                                <span class="text-sm text-gray-700">Menunggu: {{ isset($statusData) ? number_format(($statusData['pending'] / max($totalPayments, 1)) * 100, 1) : 0 }}%</span>
                            </div>
                            <div class="flex items-center">
                                <div class="w-4 h-4 bg-red-500 rounded-full mr-2"></div>
                                <span class="text-sm text-gray-700">Ditolak: {{ isset($statusData) ? number_format(($statusData['rejected'] / max($totalPayments, 1)) * 100, 1) : 0 }}%</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 bg-blue-50">
                ...
                <p class="mt-1 max-w-2xl text-sm text-gray-500">
                    Detail pembayaran per bulan dalam tahun {{ isset($selectedYear) ? $selectedYear : date('Y') }}.
                </p>
            </div>
            <div class="border-t border-gray-200 p-6">
                ...
                <div class="py-8 text-center">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Tidak ada pembayaran yang ditemukan pada tahun tersebut.</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 bg-blue-50">
                ...
                <p class="mt-1 max-w-2xl text-sm text-gray-500">
                    Daftar pembayaran yang terkait dengan laporan tahunan {{ $selectedYear ?? date('Y') }}.
                </p>
            </div>
            <div class="border-t border-gray-200 p-6">
                <div class="flex justify-between items-center mb-4">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const statusData = @json($statusData ?? ['verified' => 0, 'pending' => 0, 'rejected' => 0]);
            ...
            @else
            // Display message when no data for monthly chart
            new Chart(monthlyCtx, {
                type: 'bar',
                data: {
            ...
            // Display message when no data for status pie chart
            const statusCtx = document.getElementById('statusPieChart').getContext('2d');
            new Chart(statusCtx, {
                type: 'pie',
                data: {
